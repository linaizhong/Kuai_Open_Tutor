...
function toSubjectId(name) {
  // Standard slug: lowercase ASCII letters and digits only.
  const ascii = name.toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');

  // If the name contains non-ASCII characters (e.g. Chinese, Japanese, Arabic)
  // the ascii slug will be empty. Supplement with hex-encoded Unicode code points
  // so the ID is always unique, stable, and filesystem-safe.
  // e.g. "高中数学"  → "9ad8-4e2d-6570-5b66"
  // e.g. "HSC 数学"  → "hsc-6570-5b66"
  const hasNonAscii = /[^\x00-\x7F]/.test(name);
  if (hasNonAscii) {
    const hex = Array.from(name)
      .filter(ch => ch.codePointAt(0) > 0x7f)   // encode only non-ASCII chars
      .map(ch => ch.codePointAt(0).toString(16))
      .join('-')
      .slice(0, 48);                              // cap length for sane folder names
    return ascii.length > 0 ? `${ascii}-${hex}` : hex;
  }
...
async function callModel(model, systemPrompt, userPrompt, maxTokens = 4000) {
  const messages = [
